...
def main(datadir):
    ...
    X_halflife, X_promoter, y, geneName = file['halflifeData'], file['promoter'], file['label'], file['geneName']
    ...
    X_trainhalflife, X_trainpromoter, y_train, geneName_train = X_halflife[train_indexes], X_promoter[train_indexes], y[train_indexes], geneName[train_indexes]
    X_validhalflife, X_validpromoter, y_valid, geneName_valid = X_halflife[val_indexes], X_promoter[val_indexes], y[val_indexes], geneName[val_indexes]
...
def objective(params):
    ...
    x = Conv1D(int(params['numFiltersConv1']), int(params['filterLenConv1']), dilation_rate=int(params['dilRate1']),
               padding='same', kernel_initializer='glorot_normal', input_shape=X_trainpromoterSubseq.shape[1:],
    ...
    #model.compile(Adam(learning_rate=0.01, beta_1=0.9, beta_2=0.999, epsilon=1e-07, decay=0.2),'mean_squared_error', metrics=['mean_squared_error'])
    ...
    MODEL_NAME = "bestparams_7_new_median_adam_default"
    check_cb = ModelCheckpoint(os.path.join(params['datadir'], MODEL_NAME + ".keras"), monitor='val_loss', verbose=1,
                               save_best_only=True, mode='min')
    earlystop_cb = EarlyStopping(monitor='val_loss', patience=3, verbose=1, mode='min')
