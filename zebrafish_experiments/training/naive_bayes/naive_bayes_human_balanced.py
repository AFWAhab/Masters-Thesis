...
absent_X = [seq for (idx, seq) in enumerate(X_train) if idx in absent_idx]
present_X = [seq for (idx, seq) in enumerate(X_train) if idx in present_idx]
absent_y = [l for l in y_train if l == 0]
present_y = [l for l in y_train if l == 1][:len(absent_y)]
X_train = np.concatenate((absent_X, present_X), axis=None)
y_train = np.concatenate((absent_y, present_y), axis=None)
