...
class ExpressionPredictor(nn.Module):
    def __init__(self, embedding_dim, metadata_dim, hidden_emb=512, hidden_meta=32, combined_hidden=128):
        ...
        self.combined = nn.Sequential(
            nn.Linear(hidden_emb + hidden_meta, combined_hidden),
            nn.ReLU(),
            nn.Dropout(0.2),
...
if __name__ == "__main__":
    ...
    for run in range(1, num_runs + 1):
        print(f"\n🔥 Run {run}/{num_runs}")
        model = ExpressionPredictor(embedding_dim=emb_dim, metadata_dim=meta_train.shape[1]).to(device)
    ...
    df_logs.to_csv("output/cnn_attempt_1_dnabert1.csv", index=False,
                   columns=["run", "epoch", "train_loss", "val_loss", "val_r2", "test_r2"])
    print("\n📁 Metrics saved to output/cnn_attempt_1.csv")
